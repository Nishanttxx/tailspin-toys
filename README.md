...
The repository's detailed coding standards live in [`.github/instructions`](./.github/instructions/). In particular:

- Comments should explain intent, constraints, or non-obvious decisions — never restate code that is already clear.
- Every exported function in `db/` and `src/lib/` requires TSDoc/JSDoc describing its purpose, parameters, and return value, including injectable database arguments.
- Every reusable `.astro` component requires a documented `Props` interface that describes its public contract.
- Application TypeScript uses single quotes, semicolons, spaces inside object braces, no spaces inside array brackets, and trailing commas in multiline lists. ESLint enforces these formatting rules.
